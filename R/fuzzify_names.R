...
fuzzify_names <- function(dataset, key.dict) {
  ...
  if (exists("unmatched_indices") == FALSE) {
...
Would you like the function to proceed with this vector anyways (future edits may be difficult)? (Y/N). ")
  ...
  if (exists("custom_indices") == FALSE) {
...
Would you like the function to proceed with this vector anyways (future edits may be difficult)? (Y/N). ")
  ...
  for (i in 1:length(indices)) {
    ...
    for (k in 1:15) {
      if (is.na(kd.filtered$wrong[k]) == TRUE) {
        next
      } else {
        print(paste0(k, ": ", kd.filtered$wrong[k]))  
      }
    }
    ...
    if ((is.na(as.numeric(ans1)) == FALSE) & 
        (as.numeric(ans1) %in% c(1:15)) &
        (is.na(kd.filtered$right[as.numeric(ans1)]) == FALSE)) {
        dataset$name[ind] <- kd.filtered$right[as.numeric(ans1)]
        print(paste0("The standardized name for ", kd.filtered$wrong[as.numeric(ans1)], " is ", kd.filtered$right[as.integer(ans1)]))
        ...
        samename_inds <- which(!is.na(match(dataset$name, origname)))
        
        if (length(samename_inds) != 0) {
          dataset$name[samename_inds] <- dataset$name[ind]
          indices <- indices[-samename_inds] # removing the indices of repeat names from the list of indices to be fuzzy matched
        }
    ...
    } else if ((is.na(as.numeric(ans1)) == FALSE)) {
               #& (as.numeric(ans1) > 15 | as.numeric(ans1) < 1) 
      print("A number outside of the range of choices has been selected :(")
      print(paste0("We'll keep the original name: ", dataset$name[ind], " in the dataset and add this index to unmatched_indices."))
    ...
    ## custom name or choose not to do anything for the time being
     ...
     print(paste0("The previous name (", dataset$name[ind], ") will be kept."))
     ...
     ans2 <- readline(prompt = "Answer: ")
      ...
      if (substr(toupper(as.character(ans2)), 1, 1) == "Y") {
        ...
        dataset$name[ind] <- as.character(ans3)
        print(paste0("The name (", as.character(ans3), ") will be kept in the dataset. 
        ...
        print(paste0("We'll leave this name as it is: ", dataset$name[ind]))
        unmatched_indices[i] <<- ind
      }
