...
require(stringi)
# Data-Driven Lab
#' @title Name and unique Entity Type combination by ISO
#' @export
#' @description This function takes two data frames (\code{df1} and \code{df2}) usually the key_dict and performs a conditional cross-join.
...
#'
#' @param df1 The data frame to be cleaned containing an 'iso' column, the names of the entities and other attributes. This data frame
#'   provides the base structure for the output.
#' @param df2 The reference data frame (i.e key_dict) containing both an 'iso' column and the \code{string_column} (ent_type_eng)
...
#' @param string_column A character string specifying the name of the column in \code{df2}
#'   that contains the entity types or unique strings to be combined with \code{df1}.
#'
#' @return A new data frame where all rows from the original \code{df1} have been
...
# Define the function
entity_combine_by_iso <- function(df1, df2, string_column) {
  # Function to create a new data frame with unique strings from df2 as a column after filtering by ISO from df1
  create_df_with_unique_strings <- function(iso, df1, df2, string_column) {
    unique_strings <- df2 %>%
      filter(iso == !!iso) %>%
      pull(!!sym(string_column)) %>%
      unique()

    filtered_df1 <- df1 %>%
      filter(iso == !!iso)

    if (length(unique_strings) > 0 && nrow(filtered_df1) > 0) {
      map(unique_strings, ~ mutate(filtered_df1, !!sym(string_column) := .x)) %>%
        bind_rows()
    } else {
      NULL
    }
  ...
  # Get unique ISO codes from df1
  unique_isos <- unique(df1$iso)

  # Use purrr's map function to apply the function to each unique ISO code and create a list of data frames
  list_of_dfs <- map(unique_isos, ~ create_df_with_unique_strings(.x, df1, df2, string_column))

  # Remove any NULL elements from the list
  list_of_dfs <- compact(list_of_dfs)

  # Combine all data frames together using bind_rows
  final_df <- bind_rows(list_of_dfs)%>%
  mutate(code = NA)%>%
  mutate(name = case_when(is.na(code) ~ paste0(ent_type_eng, " of ", name),
                          !is.na(code) ~ paste0(ent_type_eng, " of ", name, ", ", code)))%>% distinct()
