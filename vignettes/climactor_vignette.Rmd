...
Apart from the data you wish to clean, two other important datasets would be the key dictionary and country dictionary. These datasets can be accessed once the `climactor` package has been called with either `library(climactor)` or `require(climactor)`, 

```{r}
head(key_dict[, 1:5])
head(country_dict[, 1:5])
```
