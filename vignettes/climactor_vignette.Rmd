...
# seems like there are some NAs in the data 
```
It seems like there are some NAs in the data. Apart from that, the only other country where there isn't an exact match is Taiwan. 
...
We want to go back to the rows which countries didn't get matched (because the countries were NAs) and check for the actors of these rows to see if these entries are correct.

```{r}
cdp$name[unmatched_count]
```
