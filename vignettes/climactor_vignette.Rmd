...
It should also be further noted that the `fuzzify_country()` function will not match for `NA` values in the country column. Users are encouraged to 

```{r, eval = F}
tmp <- fuzzify_country(cdp, country_dict)
```
