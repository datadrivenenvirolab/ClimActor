...
```{r, include=FALSE}
# Spawn a child process to run some code to work around the issue of interactivity
library(subprocess)
# Helper functions necessary for the package to run 
...
child_r <- spawn_process(R_binary(), c("--vanilla"))
Sys.sleep(2)
process_read(child_r)$stdout
code <- 
'
library(climactor)
cdp <- read.csv("../data-raw/2018_-_2019_City-wide_emissions.csv", stringsAsFactors = F,
...
tmp <- rename_col(cdp)
'
process_write(child_r, code)
process_read(child_r)$stdout
process_write(child_r, "City")
process_read(child_r)$stdout
```
