...
Apart from the data you wish to clean, two other important datasets included in the ClimActor package are the key dictionary (`key_dict`) and country dictionary (`country_dict`). These datasets can be accessed once the `ClimActor` package has been called with either `library(ClimActor)` or `require(ClimActor)`. For more details on the two dictionaries, you can use `?key_dict` and `?country_dict` for the full data documentation.  
...
```{r, echo = F, comment = ""}
cat("The original name is Rio Verde with iso code BRA and entity type Region
Here are some possible matches we found: 
1:  Rio de Janeiro (State); iso: BRA and entity type: Region
2:  Rondônia; iso: BRA and entity type: Region
3:  Rio Grande do Sul; iso: BRA and entity type: Region
4:  Ceara; iso: BRA and entity type: Region
5:  Acre; iso: BRA and entity type: Region
6:  Mato Grosso; iso: BRA and entity type: Region
7:  Minas Gerais; iso: BRA and entity type: Region
8:  São Paulo State; iso: BRA and entity type: Region
9:  Pernambuco; iso: BRA and entity type: Region
...
cat("Answer: n")
cat("The previous name (Rio Verde) will be kept.")
unmatched_indices <- name_ind[1]
```
The second example considers when a match is found within the key dictionary for the actor being cleaned. 
```{r, echo = F, comment = ""}
cat("The original name is de Chihuahua with iso code MEX and entity type City
Here are some possible matches we found: 
1:  Chihuahua; iso: MEX and entity type: City
2:  Celaya; iso: MEX and entity type: City
3:  Tamiahua; iso: MEX and entity type: City
4:  Tekax; iso: MEX and entity type: City
5:  San Miguel de Allende; iso: MEX and entity type: City
6:  Teocelo; iso: MEX and entity type: City
7:  Córdoba; iso: MEX and entity type: City
8:  Apodaca; iso: MEX and entity type: City
9:  Tuxcueca; iso: MEX and entity type: City
10: Tapalpa; iso: MEX and entity type: City
11: Totatiche; iso: MEX and entity type: City
12: Degollado; iso: MEX and entity type: City
13: Queretaro; iso: MEX and entity type: City
14: Juárez; iso: MEX and entity type: City
15: Xalapa; iso: MEX and entity type: City
Which name matches? Choose 1-15. (Type N if none match; type S to save your current progress; type C to enter a custom name)")
cat("Answer: 1")
cat("Chihuahua has been selected and will replace de Chihuahua in the database.")
cdp$name[grep("de Chihuahua", cdp$name)] <- "Chihuahua"
```
...
ind.short <- name_ind[1:3]
.actor_updates <<- data.frame(ind = ind.short,
                              name_wrong = cdp$name[ind.short])
head(.actor_updates)
```
